...
class JSCodegenVisitor extends GeneralizingAstVisitor with ConversionVisitor {
  ...
  JSMetaLet _emitOpAssign(Expression left, Expression right, String op,
    ...
    var vars = {};
    var lhs = _captureLeftHandSide(vars, left, context: context);
  ...
  JSMetaLet _emitPostfixIncrement(Expression expr, Token op) {
    ...
    // evaluated only once.
    var vars = {};
    var left = _captureLeftHandSide(vars, expr, context: expr);

    // Desugar `x++` as `(x1 = x0 + 1, x0)` where `x0` is the original value
    // and `x1` is the new value for `x`.
    var x = _captureValue(vars, 'x', left, context: expr);
  ...
  Expression _captureLeftHandSide(
    ...
    if (expr is IndexExpression) {
      ...
      return new PrefixedIdentifier(
          _captureValue(scope, 'o', ident.prefix, context: context),
          ident.period, ident.identifier)..staticType = expr.staticType;
    }
    return expr;
  }
  ...
  JS.Expression _emitPrefixExpression(Token op, Expression expr) {
    ...
    if (unaryOperationIsPrimitive(dispatchType)) {
      if (_isNonNullableExpression(expr)) {
        ...
        var vars = {};
        var x = _captureLeftHandSide(vars, expr, context: expr);
  ...
  JS.Node visitCascadeExpression(CascadeExpression node) {
    ...
    var vars = {};
    _cascadeTarget = _captureValue(vars, '_', node.target, context: node);
