dart_library.library('dart/_runtime', null, /* Imports */[
  ...
  function _ignoreTypeFailure(actual, type) {
    ...
    if (isSubtype(type, core.Iterable) && isSubtype(actual, core.Iterable) || isSubtype(type, async.Future) && isSubtype(actual, async.Future) || isSubtype(type, core.Map) && isSubtype(actual, core.Map) || isSubtype(type, core.Function) && isSubtype(actual, core.Function) || isSubtype(type, async.Stream) && isSubtype(actual, async.Stream) || isSubtype(type, async.StreamController) && isSubtype(actual, async.StreamController) || isSubtype(type, async.StreamTransformer) && isSubtype(actual, async.StreamTransformer) || isSubtype(type, async.StreamSubscription) && isSubtype(actual, async.StreamSubscription)) {
      if (false) {
