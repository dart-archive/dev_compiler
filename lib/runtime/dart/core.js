dart_library.library('dart/core', null, /* Imports */[
  ...
  class Stopwatch extends Object {
    ...
    static _now() {
      return dart.as(_js_helper.Primitives.timerTicks(), int);
    }
  }
  ...
  class Uri extends Object {
    get authority() {
      if (!dart.notNull(this.hasAuthority))
        return "";
      let sb = new StringBuffer();
      this[_writeAuthority](dart.as(sb, StringSink));
    ...
    static _makeHttpUri(scheme, authority, unencodedPath, queryParameters) {
      ...
      if (authority != null && dart.notNull(authority[dartx.isNotEmpty])) {
        ...
        if (dart.notNull(dart.as(dart.dsend(hostStart, '<', authority[dartx.length]), bool)) && authority[dartx.codeUnitAt](dart.as(hostStart, int)) == Uri._LEFT_BRACKET) {
          for (; dart.notNull(dart.as(dart.dsend(hostEnd, '<', authority[dartx.length]), bool)); hostEnd = dart.dsend(hostEnd, '+', 1)) {
            if (authority[dartx.codeUnitAt](dart.as(hostEnd, int)) == Uri._RIGHT_BRACKET)
              break;
          }
          ...
          Uri.parseIPv6Address(authority, dart.as(dart.dsend(hostStart, '+', 1), int), dart.as(hostEnd, int));
          hostEnd = dart.dsend(hostEnd, '+', 1);
          if (!dart.equals(hostEnd, authority[dartx.length]) && authority[dartx.codeUnitAt](dart.as(hostEnd, int)) != Uri._COLON) {
            dart.throw(new FormatException("Invalid end of authority", authority, dart.as(hostEnd, int)));
          }
        }
        let hasPort = false;
        for (; dart.notNull(dart.as(dart.dsend(hostEnd, '<', authority[dartx.length]), bool)); hostEnd = dart.dsend(hostEnd, '+', 1)) {
          if (authority[dartx.codeUnitAt](dart.as(hostEnd, int)) == Uri._COLON) {
            let portString = authority[dartx.substring](dart.as(dart.dsend(hostEnd, '+', 1), int));
            if (dart.notNull(dart.as(dart.dload(portString, 'isNotEmpty'), bool)))
              port = int.parse(dart.as(portString, String));
            break;
          }
        }
        host = authority[dartx.substring](dart.as(hostStart, int), dart.as(hostEnd, int));
      }
      return Uri.new({scheme: scheme, userInfo: dart.as(userInfo, String), host: dart.as(host, String), port: dart.as(port, int), pathSegments: unencodedPath[dartx.split]("/"), queryParameters: queryParameters});
    }
    ...
    static _makeWindowsFileUrl(path) {
      ...
      if (dart.notNull(path[dartx.length]) > 1 && path[dartx.get](1) == ":") {
        ...
        Uri._checkWindowsPathReservedCharacters(dart.as(pathSegments, List$(String)), true, 1);
      ...
      if (dart.notNull(path[dartx.length]) > 0 && path[dartx.get](0) == sep) {
        if (dart.notNull(path[dartx.length]) > 1 && path[dartx.get](1) == sep) {
          ...
          Uri._checkWindowsPathReservedCharacters(dart.as(pathSegments, List$(String)), true);
    ...
    static _makeQuery(query, start, end, queryParameters) {
      ...
      queryParameters.forEach(dart.fn((key, value) => {
        ...
        first = false;
        dart.dsend(result, 'write', Uri.encodeQueryComponent(dart.as(key, String)));
        if (value != null && !dart.notNull(dart.as(dart.dload(value, 'isEmpty'), bool))) {
          dart.dsend(result, 'write', "=");
          dart.dsend(result, 'write', Uri.encodeQueryComponent(dart.as(value, String)));
        }
      }));
    ...
    [_toWindowsFilePath]() {
      ...
      if (dart.notNull(dart.as(dart.dsend(dart.dload(segments, 'length'), '>', 0), bool)) && dart.equals(dart.dload(dart.dindex(segments, 0), 'length'), 2) && dart.equals(dart.dsend(dart.dindex(segments, 0), 'codeUnitAt', 1), Uri._COLON)) {
        Uri._checkWindowsDriveLetter(dart.as(dart.dsend(dart.dindex(segments, 0), 'codeUnitAt', 0), int), false);
        Uri._checkWindowsPathReservedCharacters(dart.as(segments, List$(String)), false, 1);
        hasDriveLetter = true;
      } else {
        Uri._checkWindowsPathReservedCharacters(dart.as(segments, List$(String)), false);
      }
      let result = new StringBuffer();
    ...
    static _hexCharPairToByte(s, pos) {
      ...
      for (let i = 0; dart.notNull(i) < 2; i = dart.notNull(i) + 1) {
        ...
        if (48 <= dart.notNull(dart.as(charCode, num)) && dart.notNull(dart.as(dart.dsend(charCode, '<=', 57), bool))) {
          byte = dart.asInt(dart.notNull(byte) * 16 + dart.notNull(dart.as(charCode, num)) - 48);
        } else {
          charCode = dart.dsend(charCode, '|', 32);
          if (97 <= dart.notNull(dart.as(charCode, num)) && dart.notNull(dart.as(dart.dsend(charCode, '<=', 102), bool))) {
            byte = dart.asInt(dart.notNull(byte) * 16 + dart.notNull(dart.as(charCode, num)) - 87);
          } else {
      ...
      if (dart.notNull(simple)) {
        ...
        for (let i = 0; dart.notNull(i) < dart.notNull(text[dartx.length]); i = dart.notNull(i) + 1) {
          ...
          if (dart.equals(codeUnit, Uri._PERCENT)) {
            ...
            bytes[dartx.add](dart.as(codeUnit, int));
          }
        }
      }
