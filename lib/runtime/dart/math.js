dart_library.library('dart/math', null, /* Imports */[
  ...
  class Random extends core.Object {
    static new(seed) {
      ...
      return seed == null ? dart.const_(new _JSRandom()) : new _Random(seed);
    }
  }
  ...
  const MutableRectangle$ = dart.generic(function(T) {
    ...
    MutableRectangle[dart.implements_] = () => [Rectangle$(T)];
  ...
  function min(a, b) {
    if (!(typeof a == 'number')) dart.throw_(new core.ArgumentError(a));
    if (!(typeof b == 'number')) dart.throw_(new core.ArgumentError(b));
  ...
  dart.fn(min, () => dart.definiteFunctionType(dart.dynamicR, [dart.dynamicR, dart.dynamicR]));
  function max(a, b) {
    if (!(typeof a == 'number')) dart.throw_(new core.ArgumentError(a));
    if (!(typeof b == 'number')) dart.throw_(new core.ArgumentError(b));
    if (dart.notNull(a) > dart.notNull(b)) return a;
  ...
  dart.fn(max, () => dart.definiteFunctionType(dart.dynamicR, [dart.dynamicR, dart.dynamicR]));
  function atan2(a, b) {
  ...
  class _JSRandom extends core.Object {
    ...
    nextInt(max) {
      if (dart.notNull(max) <= 0 || dart.notNull(max) > dart.notNull(_POW2_32)) {
        dart.throw_(new core.RangeError(`max must be in range 0 < max ≤ 2^32, was ${max}`));
      }
  ...
  _JSRandom[dart.implements_] = () => [Random];
